...
class Maze {
  ...
  setTaget(){
    ...
    this.grid[rand_x][rand_y].setConfig(
      this.columns,
      this.rows,
      "green"
      );
  ...
  game(){
    ...
    let dir = this.spirit.getDirection();
        ...
        console.log('top');
          switch(dir){
            case 'n':
              node = new Node(
                0,
                this.grid[current.rowNum - 1][current.colNum].flat + 0,
                current.rowNum - 1,
                current.colNum
              );
              stack.push(node);
            ...
            case 'o':
              node = new Node(
                12,
                this.grid[current.rowNum - 1][current.colNum].flat + 12,
                current.rowNum - 1,
                current.colNum
              );
              stack.push(node);
            ...
            case 's':
              node = new Node(
                8,
                this.grid[current.rowNum - 1][current.colNum].flat + 8,
                current.rowNum - 1,
                current.colNum
              );
              stack.push(node);
            ...
            case 'e':
              node = new Node(
                4,
                this.grid[current.rowNum - 1][current.colNum].flat + 4,
                current.rowNum - 1,
                current.colNum
              );
              stack.push(node);
        ...
        console.log('right');
          switch(dir){
            case 'n':
              node = new Node(
                4,
                this.grid[current.rowNum][current.colNum + 1].flat + 4,
                current.rowNum,
                current.colNum + 1
              );
              stack.push(node);
            ...
            case 'o':
              node = new Node(
                8,
                this.grid[current.rowNum][current.colNum + 1].flat + 18,
                current.rowNum,
                current.colNum + 1
              );
              stack.push(node);
            ...
            case 's':
              node = new Node(
                12,
                this.grid[current.rowNum][current.colNum + 1].flat + 12,
                current.rowNum,
                current.colNum + 1
              );
              stack.push(node);
            ...
            case 'e':
              node = new Node(
                0,
                this.grid[current.rowNum][current.colNum + 1].flat + 0,
                current.rowNum,
                current.colNum + 1
              );
              stack.push(node);
        ...
        console.log('left');
          switch(dir){
            ...
            case 'e':
              node = new Node(
                8,
                this.grid[current.rowNum][current.colNum - 1].flat + 8,
                current.rowNum,
                current.colNum - 1
              );
              stack.push(node);
    ...
    // rotate current
    this.spirit.rotate(popNode.timeRotation);
    // mark path
    current.spiritVisited = true;
    // if ending
    if(current.target){
      generationComplete = true;
      return;
    ...
    // upddate current
    current = this.grid[popNode.rowNum][popNode.colNum];
    // paint the area where i move
    current.setConfigWay(
      this.rows,
      this.columns
    );
    ...
    for (let i = 0; i < 10; i++) {
      (function (i) {
          window.setTimeout(function () {}, i * 10000);
      }(i));
    }
  }
...
class Cell {
    ...
    setConfig(columns,rows,color) {
      let x = (this.colNum * this.parentSize) / columns;
      let y = (this.rowNum * this.parentSize) / rows;
      ctx.fillStyle = color;
      ctx.fillRect(
        x,
        y,
    ...
    setConfigWay(columns,rows) {
      ...
      ctx.fillRect(
        x,
        y,
        (this.parentSize / columns) - 30,
        (this.parentSize / rows) - 30
      );
    }
...
class Node {
  constructor(timeRotation,totalTime,rowNum,colNum){
